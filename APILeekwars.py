...
class APIAi:
    ...
    def test(self, ai_id, leek_id, bots, type, token):
        url = self.url + "/test/"
        return self.session.post(url, data={"ai_id": ai_id, "leek_id": leek_id, "bots": bots, "type": type,
                                            "token": token}).json()
...
class APIAiFolder:
    ...
    def change_folder(self, folder_id, dest_folder_id, token):
        ...
        return self.session.post(url,
                                 data={"folder_id": folder_id, "dest_folder_id": dest_folder_id, "token": token}).json()
...
class APIFarmer:
    ...
    def register(self, login, password, email, leek_name, godfather):
        ...
        return self.session.post(url,
                                 data={"login": login, "password": password, "email": email, "leek_name": leek_name,
    ...
    def unregister(self, password, delete_forum_messages, token):
        ...
        return self.session.post(url, data={"password": password, "delete_forum_messages": delete_forum_messages,
                                            "token": token}).json()
...
class APIFight:
    ...
    def get_logs(self, fight_id, token):
        url = self.url + "/get-logs/" + str(fight_id)
        return self.session.get(url).json()
...
class APIGarden:
    ...
    def get_composition_opponents(self, composition, token):
        url = self.url + "/get-composition-opponents/" + str(composition)
        return self.session.get(url).json()

    def get_farmer_challenge(self, target, token):
        url = self.url + "/get-farmer-challenge/" + str(target)
        return self.session.get(url).json()

    def get_farmer_opponents(self, token):
        url = self.url + "/get-farmer-opponents"
        return self.session.get(url).json()
    ...
    def get_solo_challenge(self, leek_id, token):
        url = self.url + "/get-solo-challenge/" + str(leek_id)
        return self.session.get(url).json()
    ...
    def start_team_fight(self, composition_id, target_id, token):
        ...
        return self.session.post(url,
                                 data={"composition_id": composition_id, "target_id": target_id, "token": token}).json()
...
class APILeek:
    ...
    def get_level_popup(self, leek_id, token):
        url = self.url + "/get-level-popup/" + str(leek_id)
        return self.session.get(url).json()
    ...
    def get_private(self, leek_id, token):
        url = self.url + "/get-private/" + str(leek_id)
        return self.session.get(url).json()

    def get_registers(self, leek_id, token):
        url = self.url + "/get-registers/" + str(leek_id)
        return self.session.get(url).json()
...
class APIMessage:
    ...
    def get_latest_conversations(self, count, token):
        url = self.url + "/get-latest-conversations/" + str(count)
        return self.session.get(url).json()

    def get_messages(self, conversation_id, count, page, token):
        url = self.url + "/get-messages/" + str(conversation_id) + "/" + str(count) + "/" + str(page)
        return self.session.get(url).json()
    ...
    def send_message(self, conversation_id, message, token):
        ...
        return self.session.post(url,
                                 data={"conversation_id": conversation_id, "message": message, "token": token}).json()
...
class APINotification:
    ...
    def get_latest(self, count, token):
        url = self.url + "/get-latest/" + str(count)
        return self.session.get(url).json()
...
class APIRanking:
    ...
    def search(self, query, search_leeks, search_farmers, search_teams):
        url = self.url + "/search/"
        return self.session.post(url,
                                 data={"query": query, "search_leeks": search_leeks, "search_farmers": search_farmers,
                                       "search_teams": search_teams}).json()
...
class APITeam:
    ...
    def get_connected(self, team_id, token):
        url = self.url + "/get-connected/" + str(team_id)
        return self.session.get(url).json()

    def get_private(self, team_id, token):
        url = self.url + "/get-private/" + str(team_id)
        return self.session.get(url).json()
...
class APITournament:
    ...
    def get(self, tournament_id, token):
        url = self.url + "/get/" + str(tournament_id)
        return self.session.get(url).json()
...
class APITrophy:
    ...
    def get_admin(self, lang, token, supertoken):
        url = self.url + "/get-admin/" + lang + "/" + supertoken
        return self.session.get(url).json()
    ...
    def get_farmer_trophies(self, farmer_id, lang, token):
        url = self.url + "/get-farmer-trophies/" + str(farmer_id) + "/" + lang
        return self.session.get(url).json()
